...
//        arr[3]=p3.getAddress();
    ...
    public String selectDataMember(){
        String temp ="10000";
        for(int i=0;i<ListAdapter.num;i++){
            if(temp.compareTo(ListAdapter.likelist[i])<0){
                temp = ListAdapter.likelist[i];
            }
        }

        String q = "select* from member2 where cal='"+temp+"' order by cal";
